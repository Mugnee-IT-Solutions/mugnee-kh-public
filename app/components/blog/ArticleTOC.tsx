...
export default function ArticleTOC({ items }: ArticleTOCProps) {
  ...
  return (
    <nav aria-label="Table of contents" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-700">Table of Contents</h3>
      <p className="mt-1 text-xs text-slate-500">Core reading path</p>
      ...
      {extraItems.length ? (
        <details className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-2">
          <summary className="cursor-pointer text-xs font-semibold uppercase tracking-wide text-slate-600">
            More Sections ({extraItems.length})
          </summary>
