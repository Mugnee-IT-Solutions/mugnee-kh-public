import Head from "next/head";
import dynamic from "next/dynamic";
import SiteHeader from "../../app/components/layout/Header";
import SiteFooter from "../../app/components/layout/Footer";
import SitewideJsonLd from "../../app/components/seo/SitewideJsonLd";
import TurnstileGateClient from "../../app/products/turnstile-gate/TurnstileGateClient";
import { SITE_URL } from "../../app/lib/site";

const ClientEnhancements = dynamic(
  () => import("../../app/components/layout/ClientEnhancements"),
  { ssr: false },
);

const title = "Turnstile Gate Cambodia | Price & Installation";
const description =
  "Turnstile gate Cambodia with speed gate, flap barrier, tripod turnstile, RFID, biometric integration, installation, and quotation support.";

export default function TurnstileGatePage() {
  return (
    <>
      <Head>
        <title>{title}</title>
        <meta name="description" content={description} />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href={`${SITE_URL}/turnstile-gate/`} />
        <link rel="alternate" hrefLang="en-KH" href={`${SITE_URL}/turnstile-gate/`} />
        <link rel="alternate" hrefLang="km-KH" href={`${SITE_URL}/km/turnstile-gate/`} />
        <link rel="alternate" hrefLang="x-default" href={`${SITE_URL}/turnstile-gate/`} />
        <meta property="og:title" content={title} />
        <meta
          property="og:description"
          content="Turnstile gate and access control solutions in Cambodia with speed gate, flap barrier, RFID, biometric integration, and local installation."
        />
        <meta property="og:url" content={`${SITE_URL}/turnstile-gate/`} />
        <meta property="og:site_name" content="Mugnee Cambodia" />
        <meta property="og:type" content="website" />
        <meta property="og:image" content={`${SITE_URL}/images/hero/cambodia-led-hero.webp`} />
        <meta property="og:image:width" content="1200" />
        <meta property="og:image:height" content="630" />
        <meta property="og:image:alt" content="Turnstile Gate Solutions in Cambodia" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={title} />
        <meta
          name="twitter:description"
          content="Turnstile gate solutions in Cambodia with RFID, biometric integration, installation, and quotation support."
        />
        <meta name="twitter:image" content={`${SITE_URL}/images/hero/cambodia-led-hero.webp`} />
      </Head>

      <div className="h-screen w-full overflow-hidden">
        <SitewideJsonLd />
        <ClientEnhancements />
        <SiteHeader />
        <main
          id="app-scroll-root"
          className="mt-[var(--header-height)] h-[calc(100vh-var(--header-height))] overflow-y-auto overflow-x-hidden"
        >
          <TurnstileGateClient />
          <SiteFooter />
        </main>
      </div>
    </>
  );
}

