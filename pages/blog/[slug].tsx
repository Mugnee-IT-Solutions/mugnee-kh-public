...
function renderParagraphs(content: string) {
  const renderInlineContent = (text: string, blockKey: string) => {
    const pattern = /\[([^\]]+)\]\((\/[^)]+)\)/g;
    ...
    while (match) {
      ...
      nodes.push(
        <Link
          key={`${blockKey}-${match.index}-${match[2]}`}
          href={match[2]}
          ...
          <div className="blog-justified bg-gradient-to-b from-slate-50 via-white to-slate-50 text-slate-900">
            ...
            {post.faq?.length ? (
              <FAQJsonLd
                ...
                items={post.faq.map((item) => ({ question: item.q, answer: item.a, language: "en" as const }))}
              />
            ) : null}
